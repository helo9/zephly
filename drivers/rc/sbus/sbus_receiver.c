...
static void sbus_handle_rx(const struct device *dev) {
    ...
    uint8_t tmp;
    if ( uart_fifo_read(config->uart, &tmp, 1) < 0) {
        return;
    }
...
static void sbus_update(const struct device *dev, struct Command *rc_val) {
    struct sbus_data *data = dev->data;
    rc_val->armed = true;
    rc_val->pitch = data->received.servo_channels[0];
    rc_val->roll = data->received.servo_channels[1];
    rc_val->thrust = data->received.servo_channels[3];
    rc_val->yaw = data->received.servo_channels[4];
}
