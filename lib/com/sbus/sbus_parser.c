...
uint16_t sbus_read_next_servo_chan(struct SBusParsingCursor *cursor, const uint8_t *raw_buffer) {
...
int sbus_parse_buffer(struct SBusBuffer *buffer, struct SBusData *data) {
    ...
    struct SBusParsingCursor buffer_ptr = {};
    const uint8_t *raw_data_buffer = &buffer->data[1];

    for (int servo_chan=0; servo_chan<SBUS_NUM_SERVO_CHANNELS; servo_chan++) {
        
        data->servo_channels[servo_chan] = sbus_read_next_servo_chan(&buffer_ptr, raw_data_buffer);
    }
